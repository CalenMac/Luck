...
bool playGame(int guesses, int number)
{
	...
	for (int count = 1; count <= guesses; count += 1)
	{
...
int main()
{
	...
	const int guesses = 5; // the user has this many guesses

	do // while the user still wants to play
	{
		int number = die(mersenne); // this is the number the user needs to guess

		std::cout << "Hi I am CalenBot! I'm thinking of a number 1-100.  You have " << guesses << " tries to guess what it is.\n";

		bool won = playGame(guesses, number);
		if (won)
			std::cout << "Correct!  You win!\n";
		else
			std::cout << "Sorry, you lose. Is this Taz? The correct number was " << number << "\n";
